...
"""
    ...
    token_found = mock_token in payload_str

    # Log the proof: the token itself and the surrounding context in the payload
    token_index = payload_str.find(mock_token)
    surrounding = payload_str[max(0, token_index - 100):token_index + len(mock_token) + 100] if token_found else ""

    log_evidence("injected_token", mock_token)
    log_evidence("token_found_in_payload", token_found)
