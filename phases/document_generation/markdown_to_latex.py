...
class MarkdownToLaTeX:
    ...
    def _convert_unicode_to_latex(text: str) -> str:
        ...
        then strip any remaining non-Latin Unicode characters that would crash pdflatex."""
        for unicode_char, latex_equiv in MarkdownToLaTeX.UNICODE_TO_LATEX.items():
            text = text.replace(unicode_char, latex_equiv)
        # Strip characters outside Basic Latin + Latin Extended (U+0000–U+024F)
        # that pdflatex cannot handle (Cyrillic, CJK, Arabic, etc.)
        text = re.sub('[^\u0000-\u024F]', '', text)
        return text
    ...
    def _escape_special_chars_outside_math(text: str) -> str:
        ...
        protected_pattern = re.compile(
            ...
            r'\$[^$]+?\$'           # inline math
            r'|'
            r'\\[a-zA-Z]+\*?(?:\[[^\]]*\])?\{[^}]*\}'  # \command[...]{...} (optional [] args, single-level braces)
            r'|'
