...
class RBQLAgent:
    def __init__(self):
        ...
        self.num_of_states  = 13*12*2*2*12
        self.num_of_actions = 2
        
        # Q-table (small initial values)
        self.q_values = np.random.rand(self.num_of_states, self.num_of_actions) / 1000.0
    ...
    def get_action(self, state):
        ...
        best_action_index = int(np.argmax(self.q_values[int(state)]))
...
def update_environment(x_ball, y_ball, vx_ball, vy_ball, x_racket, action):
    """
    Perform one step of the environment physics.
    Returns: (x_ball, y_ball, vx_ball, vy_ball, x_racket, reward, done)
    """
    # perform action
    x_racket += action
    ...
    # Ceiling/Floor collision (top/bottom)
    # y=12 is bottom (terminal), y=0 would be top (if range 1-12)
    ...
    if y_ball == 12:  # terminal state reached
        ...
        # Racket width is implicit (looks like 5 units: x to x+4)
        reward = -1
        if x_ball >= x_racket and x_ball <= x_racket + 4:
            reward = +1
            
    return x_ball, y_ball, vx_ball, vy_ball, x_racket, reward, done
...
if __name__ == "__main__":
    # INITIALISE PYGAME
    import os
    # Default to windowed mode if running directly, but respected if running from experiment runner
    # (Though experiment runner prevents this block from running usually)
    ...
    while cont:
        ...
        # Use agent to get action
        action = agent.get_action(state)              # -1 or +1
        action_index = (action + 1)//2                # 0 or 1

        # perform action, move ball, check collisions
        x_ball, y_ball, vx_ball, vy_ball, x_racket, reward, done = update_environment(
            x_ball, y_ball, vx_ball, vy_ball, x_racket, action
        )
        ...
        if done:
            ...
            agent.model.add_transition(state, action_index, next_state, reward)
            
            # RBQL: propagate backwards through entire model
            agent.propagate_reward(next_state)
